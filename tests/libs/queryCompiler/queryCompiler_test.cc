...
// ///////// PostingList helpers and QueryCompiler mock class /////////
...
//    // Account for urls and title
//    // each "url" is of size 7, "title" is size 7
//    // bytes += docs.size() * (7 + 7); 
...
// class MockQueryCompiler : public QueryCompiler {
//    public:

//    MockQueryCompiler() {
//       bagOfWords = new Map<string, uint32_t>();
//       (*bagOfWords)["quick"] = 1;
//       (*bagOfWords)["brown"] = 1;
//       (*bagOfWords)["fox"] = 1;
//    }

//    // Since we don't have a real index in this test
//    virtual ISRWord* getWordISR(const string& word) override {
//       const size_t NUM_DOCS = 9;
      
//       PostingList * endDoc =  getEndDocPostingList();
//       if (word == "quick")
//          return new ISRWord(getQuickPostingList(NUM_DOCS), endDoc);
//       else if (word == "brown")
//          return new ISRWord(getBrownPostingList(NUM_DOCS), endDoc);
//       else if (word == "fox")
//          return new ISRWord(getFoxPostingList(NUM_DOCS), endDoc);
//       else
//          return nullptr;
//    }
// };
...
//     setenv("BBG_STOPWORDS", "src/libs/naughtyFilter/stopwords.txt", 1);
//     setenv("BBG_BLACKLIST", "src/libs/naughtyFilter/blacklist.txt", 1);
// }

// TEST_CASE("Tokenize function", "[queryCompiler]") {
//    setupEnvironment();
//    MockQueryCompiler qc;
...
//    REQUIRE(qc.query == expected);

//    query = "quick AND brown AND fox";
//    expected.clear();
//    expected.push_back("quick");
...
//    query = "quick OR brown || fox | horse";
//    expected.clear();
//    expected.push_back("quick");
//    expected.push_back("OR");
//    expected.push_back("brown");
//    expected.push_back("||");
//    expected.push_back("fox");
//    expected.push_back("|");
//    expected.push_back("horse");
//    qc.tokenize(query);
...
// TEST_CASE("Words not in BoW fault tolerance", "[queryCompiler]") {
//    setupEnvironment();
//    MockQueryCompiler qc;
...
// /* 
//    The functionality should be the same as in the ISR tests.
//    The difference is that here we start with a query string
//    instead of constructing the ISR objects directly.
// */

// TEST_CASE("Single word 'quick' query", "[queryCompiler]") {
//    setupEnvironment();
//    MockQueryCompiler qc;
//    string query = "quick";

//    ISR* quickISR = qc.parseQuery(query);

//    REQUIRE(quickISR != nullptr);
...
// TEST_CASE("'( quick OR brown OR fox )' query", "[queryCompiler]") {
//    setupEnvironment();
//    MockQueryCompiler qc;
//    string query = "( quick OR brown OR fox )";

//    ISR* orISR = qc.parseQuery(query);

//    REQUIRE(orISR != nullptr);
...
// TEST_CASE("'( quick AND fox )' query", "[queryComiler]") {
//    setupEnvironment();
//    MockQueryCompiler qc;
//    string query = "( quick AND fox )";

//    ISR* andISR = qc.parseQuery(query);

//    REQUIRE(andISR != nullptr);
...
// TEST_CASE("'( quick AND fox ) OR brown' query", "[queryCompiler]") {
//    setupEnvironment();
//    MockQueryCompiler qc;
//    string query = "( quick AND fox ) OR brown";
...
// TEST_CASE("'( quick AND brown ) OR ( brown AND fox )' query", "[queryCompiler]") {
//    setupEnvironment();
//    MockQueryCompiler qc;
//    string query = "( quick AND brown ) OR ( brown AND fox )";
